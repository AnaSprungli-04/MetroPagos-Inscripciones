...
def payment_success():
    ...
    return render_template('success.html', 
                           message="Â¡Tu pago fue procesado con Ã©xito! Por favor, verifica tu correo o continÃºa con los pasos adicionales.", 
        ...
        try:
            ...
            if payment_info and payment_info["status"] == 200:
                ...
                if payment_status == 'approved':
                    app.logger.info(f"Pago {payment_id} APROBADO. Actualizando estado de inscripciÃ³n para {external_reference}.")
                    # LÃ³gica para marcar la inscripciÃ³n como pagada en tu DB
                elif payment_status == 'pending':
                    app.logger.info(f"Pago {payment_id} PENDIENTE. Actualizando estado de inscripciÃ³n para {external_reference}.")
                    # LÃ³gica para marcar la inscripciÃ³n como pendiente en tu DB
                elif payment_status == 'rejected':
                    app.logger.info(f"Pago {payment_id} RECHAZADO. Actualizando estado de inscripciÃ³n para {external_reference}.")
            ...
            app.logger.error(f"ExcepciÃ³n al procesar webhook de pago {resource_id}: {e}")
...
def admin_save():
    ...
    # Sin precio base; el precio es por clase
    ...
    for idx, cls in enumerate(classes):
        ...
        try:
            price = int(price_val) if price_val not in (None, "") else cls.get('price')
        except Exception:
            price = cls.get('price')
        if name:
    ...
    if 'logo' in request.files:
        ...
        if file and file.filename:
            ...
            ext_ok = ("." in filename and filename.rsplit(".", 1)[1].lower() in {"png","jpg","jpeg","gif","webp"})
            if ext_ok:
